...
module.exports=function createPortal({db,secret,mailer}){
 ...
 router.get('/dashboard',asyncRoute(async(req,res)=>{
  ...
  const [people]=await peopleQuery.clone().count('* as count');const [active]=await peopleQuery.clone().where({status:1}).count('* as count');const requestsQuery=db('portal_requests').where({status:'pending'}).whereNot('userId',req.user.userId);if(!permitted(req,'requests:review'))requestsQuery.where({userId:req.user.userId});const [requests]=await requestsQuery.count('* as count');const [inactive]=await peopleQuery.clone().where({status:0}).count('* as count');
 ...
 const receiveAvatar=(req,res,next)=>avatarUpload(req,res,error=>{if(error){error.status=400;error.message=error.code==='LIMIT_FILE_SIZE'?'Choose an image no larger than 5 MB.':'Upload one PNG or JPEG profile photo.';return next(error);}next();});
 async function saveAvatar(req,value){await db.transaction(async trx=>{const user=await trx('employees').where({userId:req.user.userId}).forUpdate().first();const profile=parse(user.profile);profile.avatarDataUrl=value;await trx('employees').where({userId:req.user.userId}).update({profile:JSON.stringify(profile)});});}
 router.post('/me/avatar',receiveAvatar,asyncRoute(async(req,res)=>{
  const file=req.file;if(!file)fail(400,'Choose a PNG or JPEG profile photo.');
  const b=file.buffer;const png=b.length>32&&b.subarray(0,8).equals(Buffer.from([137,80,78,71,13,10,26,10]))&&b.subarray(-8,-4).toString()==='IEND';
  const jpeg=b.length>4&&b[0]===255&&b[1]===216&&b[2]===255&&b[b.length-2]===255&&b[b.length-1]===217;
  if(!((png&&file.mimetype==='image/png')||(jpeg&&file.mimetype==='image/jpeg')))fail(400,'Choose a valid PNG or JPEG image.');
  const avatarDataUrl='data:'+file.mimetype+';base64,'+b.toString('base64');await saveAvatar(req,avatarDataUrl);ok(res,{avatarDataUrl},'Profile photo updated.');
 }));
 router.post('/me/avatar/remove',asyncRoute(async(req,res)=>{await saveAvatar(req,null);ok(res,{avatarDataUrl:null},'Profile photo removed.');}));
 router.put('/me',asyncRoute(async(req,res)=>{
  const data={firstName:text(req.body.firstName,'First name',50),lastName:text(req.body.lastName,'Last name',50,false)};
  ...
  let result;await db.transaction(async trx=>{const user=await trx('employees').where({userId:req.user.userId}).forUpdate().first();const profile=parse(user.profile);
    ...
    if(data.userName&&data.userName!==user.userName){await trx('admin_login').where({empId:user.empId}).update({adminLoginName:data.userName});await trx('employee_login').where({empId:user.empId}).update({empLoginName:data.userName});}
    result=personalDetails(await trx('employees').where({userId:user.userId}).first());
 ...
 router.post('/me/password',asyncRoute(async(req,res)=>{
  ...
  await db.transaction(async trx=>{
    ...
    await trx('portal_sessions').where({userId:req.user.userId}).whereNot('id',req.session.id).whereNull('revokedAt').update({revokedAt:trx.fn.now()});
    await trx('portal_password_resets').where({userId:req.user.userId}).whereNull('consumedAt').update({consumedAt:trx.fn.now()});
  });ok(res,{updated:true},'Password changed. Other sessions have been signed out.');
 }));
 ...
 async function resource(req){const kind=req.params.kind;let table,pk='id',columns,search,query;
  ...
  query=db(table);if(kind==='login-history')query.whereNot('empId',req.user.empId);if(kind==='employees'&&req.method==='GET')query.whereNot('userId',req.user.userId);if(table==='portal_catalog')query.where({kind});if(kind==='requests'&&!permitted(req,'requests:review'))query.where({userId:req.user.userId});if(kind==='notifications')query.where(function(){this.where('recipientId',req.user.userId).orWhere(function(){this.whereNull('recipientId').whereIn('audience',req.user.roleName==='admin'?['all','admin']:['all']);});});return {kind,table,pk,columns,search,query};
 }
 ...
 router.get('/employees/:empId',asyncRoute(async(req,res)=>{const empId=text(req.params.empId,'Employee ID',50);if(empId!==req.user.empId)requirePermission(req,'employees:read');const employee=await db('employees').where({empId}).first();if(!employee)fail(404,'Employee not found.');for(const key of ['address','profile'])employee[key]=parse(employee[key],null);employee.avatarDataUrl=employee.profile&&employee.profile.avatarDataUrl||null;employee.mobile=employee.profile&&(employee.profile.mobile||employee.profile.Mobile)||null;employee.dateOfBirth=employee.profile&&(employee.profile.dateOfBirth||employee.profile.DateOfBirth)||null;delete employee.profile;const normalize=row=>{if(!row)return null;for(const key of Object.keys(row)){if(key.endsWith('Info'))row[key]=parse(row[key],null);}return row;};const basic=normalize(await db('employee_basic_details').where({empId}).first());const bank=normalize(await db('employee_bank_details').where({empId}).first());if(bank){delete bank.atmCardInfo;delete bank.personalBanksInfo;if(bank.accountNumber)bank.accountNumber='•••• '+String(bank.accountNumber).slice(-4);}
